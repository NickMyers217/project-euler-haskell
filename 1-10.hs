...
--Problem 7
    ...
    --See problem 10 for better implementation
...
--Problem 9
    euler9 cap =
        product (head (filter (\[a, b, c] -> (a^2 + b^2 == c^2)) lst))
        where lst = [ [a, b, c] | a <- [1..cap], b <- [1..cap], c <- [cap - a - b] ]

--Problem 10
    isPrimeBetter 1 = False
    isPrimeBetter 2 = True
    isPrimeBetter 3 = True 
    isPrimeBetter n
        | even n         = False
        | hasDivisor n 3 = False
        | otherwise      = True
        where 
            hasDivisor n i
                | i > truncate (sqrt (fromIntegral n)) + 1 = False
                | n `mod` i == 0                           = True
                | otherwise                                = hasDivisor n (i + 2)

    euler10 cap = sum (filter (isPrimeBetter) [3,5..cap]) + 2
